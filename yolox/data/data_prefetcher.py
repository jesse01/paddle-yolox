...
class DataPrefetcher:
    ...
    It could speedup your paddle dataloader. For more information, please check
    ...
    def next(self):
        ...
        if target is not None:
            target.record_stream(paddle.device.cuda.current_stream())
        self.preload()
        return input, target
    ...
    def _record_stream_for_image(input):
        input.record_stream(paddle.device.cuda.current_stream())


def random_resize(data_loader, exp, epoch, rank, is_distributed):
    tensor = paddle.empty([1], dtype=paddle.int64)
    if is_distributed:
        synchronize()
